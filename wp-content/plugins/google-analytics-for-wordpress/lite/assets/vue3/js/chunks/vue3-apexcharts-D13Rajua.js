import { X as defineComponent, Y as h, aB as onBeforeMount, y as onMounted, aC as getCurrentInstance, z as onBeforeUnmount, C as watch, j as ref, x as nextTick, aD as toRefs } from "./toastStore-BFDhxKQh.js";
let __miSVG;
let __miApex = {};
...
class Config {
  ...
  init({ responsiveOverride }) {
    ...
    if (opts && typeof opts === "object") {
      ...
      this.checkForDarkTheme(__miApex);
      this.checkForDarkTheme(opts);
      opts.xaxis = opts.xaxis || __miApex.xaxis || {};
      ...
      if (((_h = opts.chart.sparkline) == null ? void 0 : _h.enabled) || ((_j = (_i = __miApex.chart) == null ? void 0 : _i.sparkline) == null ? void 0 : _j.enabled)) {
        chartDefaults = defaults.sparkline(chartDefaults);
      }
      newDefaults = Utils$1.extend(config, chartDefaults);
    }
    let mergedWithDefaultConfig = Utils$1.extend(newDefaults, __miApex);
  ...
  extendYAxis(opts, w) {
    ...
    if (opts.yaxis.constructor !== Array && __miApex.yaxis && __miApex.yaxis.constructor !== Array) {
      opts.yaxis = Utils$1.extend(opts.yaxis, __miApex.yaxis);
    }
...
class Fill {
  ...
  clippedImgArea(params) {
    ...
    elImage.setAttributeNS(__miSVG.xlink, "href", fillImg);
...
class Localization {
  ...
  setCurrentLocaleValues(localeName) {
    ...
    if (__miApex.chart && __miApex.chart.locales && __miApex.chart.locales.length > 0) {
      locales = this.w.config.chart.locales.concat(__miApex.chart.locales);
    }
...
class Legend {
  ...
  createLegendMarker({ i, fillcolor }) {
    ...
    if (w.config.legend.markers.customHTML) {
      ...
      const SVGMarker = __miSVG().addTo(elMarker).size("100%", "100%");
...
if (Environment.isBrowser() && typeof __miSVG === "undefined") {
  __miSVG = SVG;
}
...
class Core {
  ...
  setupElements() {
    ...
    const SVG2 = Environment.isBrowser() ? __miSVG : global.SVG;
...
if (Environment.isBrowser()) {
  if (typeof __miSVG === "undefined") {
    __miSVG = SVG;
  }
  if (typeof __miApex === "undefined") {
    __miApex = {};
  }
} else {
...
class ApexCharts {
  ...
  render() {
    return new Promise((resolve, reject) => {
      ...
      if (Utils$1.elementExists(this.el)) {
        if (typeof __miApex._chartInstances === "undefined") {
          __miApex._chartInstances = [];
        }
        if (this.w.config.chart.id) {
          __miApex._chartInstances.push({
  ...
  destroy() {
    ...
    if (chartID) {
      __miApex._chartInstances.forEach((c, i) => {
        if (c.id === Utils$1.escapeString(chartID)) {
          __miApex._chartInstances.splice(i, 1);
        }
      });
    }
   ...
   */
  getGroupedCharts() {
    return __miApex._chartInstances.filter((ch) => {
      if (ch.group) {
        return true;
      }
  ...
  static getChartByID(id) {
    ...
    if (!__miApex._chartInstances) return void 0;
    const c = __miApex._chartInstances.filter((ch) => ch.id === chartId)[0];
    return c && c.chart;
  }
  /**
   ...
   * The chartID is used for mapping the instance stored in __miApex._chartInstances global variable
   *
